<?php
// student_dashboard.php - Student Violations Dashboard
include 'config.php'; // DB connection
include 'performance_config.php';

// Connect to violations database instead of rfid_system
$conn = getViolationDatabaseConnection();
...
if ($cachedData === false) {
    // Use violation database connection
    if (!isset($conn)) {
        $conn = getViolationDatabaseConnection();
    }
    ...
    // Add conditions based on filters (using student_violation_db column names)
    if (!empty($year)) {
    ...
    // Build final query with pagination (using student_violation_db columns)
    $whereClause = !empty($whereConditions) ? " WHERE " . implode(" AND ", $whereConditions) : "";
    $query = "SELECT SQL_CACHE id, student_id, student_name, year_level, course, section FROM students" . $whereClause . " ORDER BY student_name ASC LIMIT ? OFFSET ?";
    ...
    // Close violation database connection
    $conn->close();
}
...
<div class="dashboard">
    ...
    <form method="GET" class="filters">
        ...
        <select name="strand" id="strand">
            ...
            <option <?= $strand=="BSIT"?'selected':'' ?>>BSIT</option>
        ...
        <!-- Search Input -->
        <input type="text" name="search" placeholder="Search Student ID" value="<?= htmlspecialchars($search) ?>"
...
<script>
// Sections mapping for student_violation_db
const sections = {
    "BSCS": {
    ...
    "BSIT": {
        "1st Year": ["IT1MA","IT2MA","IT1AA","IT2AA","IT1EA","IT2EA"],
        "2nd Year": ["IT3MA","IT4MA","IT3AA","IT4AA","IT3EA","IT4EA"],
        "3rd Year": ["IT5MA","IT6MA","IT5AA","IT6AA","IT5EA","IT6EA"],
        "4th Year": ["IT7MA","IT8MA","IT7AA","IT8AA","IT7EA","IT8EA"]
    },
    "BSEntrep": {
